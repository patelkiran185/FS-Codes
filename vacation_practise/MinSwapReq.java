...
class MinSwapReq{
    ...
    public static int func(String str){
        ...
        char[] arr=new char[n];
        Arrays.fill(arr,'0');
        String initString= new String(arr);
        // System.out.println(initString);
        //initial string
        
        for(int i=0;i<n;i++){
            if(initString.charAt(i)!=str.charAt(i)){
                c++;
                initString=flip(initString,i);
            }
        }

            // approach
            // i will take the initial string and swap all its character to i
            // then i will compare the indexes of swapped and original string and then again swap and count the operations
            // maintain the minimum cariable
        

        return c;
    }
    public static String flip(String str,int i){
        StringBuilder flipp=new StringBuilder(str);
