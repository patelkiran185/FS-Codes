...
public class TreasureVault{
    public static void main(String args[]){
      ...
      int[] res=solve(boxes,n,k,x,f);
      System.out.println(Arrays.toString(res));
        sc.close();
    }
    public static int[] solve(int[] boxes,int n,int k,int x,int f){
        int[] prioritySums=new int[n-k+1];
        for(int i=0;i<=n-k;i++){
            Map<Integer,Integer> freMap=new HashMap<>();
            for(int j=i;j<i+k;j++){
                freMap.put(boxes[j],freMap.getOrDefault(boxes[j],0)+1);
            }
            ...
            for(Map.Entry<Integer,Integer> entry:freMap.entrySet()){
                ...
                int psc=fr*(int) Math.pow(t,f);
                pq.offer(new int[]{t,psc});
            }
            int sum=0;
            int count=0;
            while(!pq.isEmpty() && count<x){
                int[] treasureInfo=pq.poll();
                sum+=treasureInfo[0]*freMap.get(treasureInfo[0]);
                count++;
            }
            prioritySums[i]=sum;
        }
