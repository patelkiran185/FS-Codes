...
import java.util.*;
class Checkwhowins{
    public static boolean checkwhowins(String input){
        return backtrack(input.toCharArray());
    }
    public static boolean backtrack(char[] word){
    ...
    public static void main(String args[]){
        ...
        System.out.print(checkwhowins(input));
        sc.close();
    }
}
...
import java.util.*;
class Sol{
    public static boolean canRameshWin(String word){
        return backtrack(word.toCharArray());
    }
    static boolean backtrack(char[] word){
        for(int i=0;i<word.length-1; ++i){
            if(word[i] == 'A' && word[i+1]== 'A'){
                word[i] = 'B';
                word[i+1] = 'B';
                boolean oppLose = !backtrack(word);
                word[i] = 'A';
                word[i+1] = 'A';
                if(oppLose){
                    return true;
    ...
    public static void main(String args[]){
        ...
        System.out.print(canRameshWin(word));
    }
}
