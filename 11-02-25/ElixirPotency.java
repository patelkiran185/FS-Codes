...
class ElixirPotency {
    private static int getElixirPotency(int n, int k, int m, int[] plants) {
        List<Integer> res = new ArrayList<>();
        
        // Iterate over all possible subarray lengths starting from m to n
        for (int i = m; i <= n; i++) {
            // Iterate over all possible starting indices for the current subarray length
            for (int j = 0; j <= n - i; j++) {
                int sum = 0;
    ...
    public static void main(String args[]) {
        ...
        int result = getElixirPotency(n, k, m, plants);
